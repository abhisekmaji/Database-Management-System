...
void deletion(FileHandler &fh1, int num, char* filename, FileManager &fm );
...
int main(int argc, char** argv){
    ...
    while (getline(qfile, text)){
        ...
        ss >> words;
        ss >> words;
        deletion(fh1,stoi(words),inputfile,fm);
    }    
    qfile.close();
    //print_file(fh1,2);
    fm.CloseFile(fh1);
...
void deletion(FileHandler &fh1, int num, char* filename, FileManager &fm ){
    FileHandler fh2 = fm.OpenFile(filename);
    PageHandler ph1;
    PageHandler ph2;
    int* data1;
    int* data2;
    ...
    int page_p = binary_search_page(fh1,num,lastpage);
    if(page_p==-1){
